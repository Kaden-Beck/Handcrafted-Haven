...
import { ShoppingCart, DollarSign, Star, Package, MessageSquare, LogOut } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#023047] text-white px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="flex items-center justify-between mb-10">
        <div>
          ...
          <p className="text-2xl font-bold text-[#ffb703]">{session?.user?.name || 'Artisan'}</p>
        </div>
      ...
      {/* Hero Banner */}
      <div className="bg-linear-to-r from-[#fb8500] to-[#ffb703] rounded-3xl p-10 mb-10 shadow-2xl">
        <h1 className="text-4xl sm:text-5xl font-black tracking-tight text-white">
          Good morning, Artisan
        </h1>
        <p className="mt-4 text-xl font-medium text-white/95">Your craft is on fire today.</p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <ActivityCard title="Recent Orders" icon={<ShoppingCart className="w-6 h-6" />}>
        ...
        <ActivityCard title="Recent Reviews" icon={<Star className="w-6 h-6" />}>
        ...
        <ActivityCard title="Recent Messages" icon={<MessageSquare className="w-6 h-6" />}>
...
function StatCard({
  ...
  return (
    <div
      className={`${bg} rounded-3xl p-7 shadow-2xl border-4 border-white/20 hover:scale-105 hover:shadow-3xl transition-all duration-300`}
    >
